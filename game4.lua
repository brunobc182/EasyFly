...
function scene:create( event )
  ...
  local somBG = audio.loadStream( "sound/Koopa-Troopa-Beach.mp3" )
  audio.play(somBG, {loops = -1, channel = 4})  
...
function setupBG( )
  ...
  bg = display.newImageRect("image/bgGame3.png", _W, _H)
  bg.x = _W2
  bg.y = _H2
  scene.view:insert( bg)

  bg1 = display.newImage("image/boatGame31.png")
  bg1.x = _W2
  bg1.y = _H - 45
  scene.view:insert( bg1 )

  bg2 = display.newImage("image/boatGame3.png")
  bg2.x = bg1.x + _W
  bg2.y = _H - 45
  scene.view:insert( bg2 )

  bg3 = display.newImage("image/islandGame3.png")
  bg3.x = bg2.x + _W
  bg3.y = _H - 50
...
function createBlocks(event)
  ...
  local options = { width = 74, height = 58, numFrames = 4}
  local playerSheet = graphics.newImageSheet( "image/blocksheet3.png", options )
  local sequenceData = {
